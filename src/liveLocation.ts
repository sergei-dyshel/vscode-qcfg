...
import { TextDocument, Range, Position, ExtensionContext, workspace, TextDocumentChangeEvent, Uri } from "vscode";
...
export class LiveRange extends LiveLocation {
  ...
  adjust(document: TextDocument, change: TextDocumentContentChangeEvent):
    ...
    this.start = adjustOffsetAfterChange(
        this.start, change, false /* not end */, this.mergeOnReplace)!;
    this.end = adjustOffsetAfterChange(
        this.end, change, true /* is end */, this.mergeOnReplace)!;
...
export function adjustOffsetAfterChange(
    offset: number, change: TextDocumentContentChangeEvent, isEnd = false,
    mergeOnReplace = false): number|undefined {
