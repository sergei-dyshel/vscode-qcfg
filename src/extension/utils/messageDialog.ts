...
export namespace MessageDialog {
  ...
  /** Either title or tuple of [title, detail] (*detail* is rendered less prominent) */
  type Message = string | [message: string, detail: string];

  /**
   * Show modal message of given severity, let user pick an option.
   *
   * Should be used as more type-safe version of `window.show...Message` family of functions
   * (e.g. {@link window.showInformationMessage})
   *
   * @param severity {@link Severity}
   * @param message {@link Message}
   * @param items Array of actions. For type-safety use `[a, b] as const` notation.
